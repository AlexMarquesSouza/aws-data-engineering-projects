# Planejador de shards do Kinesis

> AWS 008 · intermediário · streaming e capacidade

Calcula shards mínimos por registros e volume por segundo e sinaliza produtores com poucas partition keys.

![Arquitetura](docs/arquitetura.svg)

```bash
python3 -m src.planner
...
| Item | Função |
|---|---|
| Python/CSV | Capacity planning local |
| Kinesis Data Streams | Stream futuro |
| Shard | Unidade de capacidade |
| Partition key/MD5 | Distribuir registros |
| `unittest`/SVG | Validação e diagrama |

Aplica throughput, capacidade provisionada, cardinalidade e risco de hot shard. Requer Python 3.10+ e custa zero localmente; Kinesis cobra conforme modo/capacidade. Foram calculados dois shards e detectado risco no produtor `partner`.

## Tecnologias relacionadas ainda não utilizadas
Sem stream, SDK, on-demand, consumers, enhanced fan-out, resharding ou CloudWatch.

## Referências oficiais
- [Conceitos do Kinesis](https://docs.aws.amazon.com/streams/latest/dev/key-concepts.html)
- [Dimensionar um stream](https://docs.aws.amazon.com/streams/latest/dev/how-do-i-size-a-stream.html)
...
cd "008-kinesis-shard-planner"
```

Confirme que `pwd` termina em `008-kinesis-shard-planner`. Os caminhos relativos usados pelo código dependem disso.
