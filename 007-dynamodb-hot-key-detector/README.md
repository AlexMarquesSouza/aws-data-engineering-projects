# Detector de hot partition keys no DynamoDB

> AWS 007 · intermediário · NoSQL e capacidade

Compara `user_id`, `status` e `event_date` pela concentração de RCU. O exemplo mostra que a data concentra 100% do tráfego e recomenda `user_id` como hipótese mais distribuída.

![Arquitetura](docs/arquitetura.svg)

## Executar e testar
```bash
python3 -m src.hotkey
...
| Item | Função |
|---|---|
| Python 3 e CSV | Análise local das requisições |
| Amazon DynamoDB | Banco NoSQL gerenciado futuro |
| Partition key | Determinar distribuição física dos itens |
| RCU | Representar atividade de leitura |
| Hottest share | Quantificar concentração de tráfego |
| Write sharding | Evolução para chaves naturalmente concentradas |
| `unittest` e SVG | Teste e documentação visual |

## Conceitos aplicados
- distribuição uniforme e hot partitions;
- cardinalidade versus popularidade real;
- throttling e eficiência de capacidade;
- análise do workload antes da criação da tabela.

## Pré-requisitos, custos e validação
Python 3.10+, custo local zero. Foi detectada concentração total em `event_date` e escolhida `user_id`. DynamoDB gera custos de capacidade, armazenamento e recursos adicionais quando criado.

## Tecnologias relacionadas ainda não utilizadas
Não há tabela, SDK, CloudWatch, WCU, adaptive capacity, sort key, GSI, auto scaling ou provisionamento.

## Referências oficiais
- [Distribuir workload com partition keys](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/bp-partition-key-uniform-load.html)
- [Partições e distribuição no DynamoDB](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/HowItWorks.Partitions.html)

## Publicação
Rascunho local; nenhuma tabela ou conta AWS foi alterada.
...
cd "007-dynamodb-hot-key-detector"
```

Confirme que `pwd` termina em `007-dynamodb-hot-key-detector`. Os caminhos relativos usados pelo código dependem disso.
