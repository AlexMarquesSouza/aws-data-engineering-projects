# Auditor de política IAM para jobs AWS Glue

> AWS 012 · intermediário · menor privilégio

Detecta ações ou recursos curinga e `iam:PassRole` sem condição em uma política de função de execução do Glue.

![Arquitetura](docs/arquitetura.svg)

```bash
python3 -m src.auditor # retorna 2 pelos wildcards intencionais
...
| Item | Função |
|---|---|
| Python / JSON | Auditor estático local |
| AWS IAM | Define ações e recursos autorizados |
| AWS Glue job role | Identidade temporária da carga ETL |
| ARN e Condition | Restringem recurso e contexto |

## Conceitos de Engenharia de Dados aplicados

Least privilege, roles de workload, policy as code, blast radius e separação entre plano de controle e acesso aos dados.

## Pré-requisitos e possíveis custos

Python 3.10+; IAM não tem custo adicional e a análise é local. Glue, S3 e CloudWatch podem cobrar quando usados; não há chamadas AWS.

## O que foi validado

Três testes cobrem política delimitada, dois tipos de wildcard e `PassRole` sem condição. A amostra possui dois achados intencionais.

## Tecnologias relacionadas ainda não utilizadas

Sem boto3, IAM Access Analyzer, SCP, permissions boundary, Lake Formation, CloudTrail, alteração de role ou deploy.

## Referências oficiais

- [IAM para AWS Glue](https://docs.aws.amazon.com/glue/latest/dg/security-iam.html)
- [Exemplos e boas práticas de políticas](https://docs.aws.amazon.com/glue/latest/dg/security_iam_id-based-policy-examples.html)
...
cd "012-glue-job-iam-policy-auditor"
```

Confirme que `pwd` termina em `012-glue-job-iam-policy-auditor`. Os caminhos relativos usados pelo código dependem disso.
