# Retentativas e dead-letter com Amazon SQS

> AWS 004 · intermediário · mensageria resiliente

Simula um consumidor que processa mensagens válidas, recupera uma falha transitória na segunda tentativa e envia uma mensagem problemática à DLQ após três recebimentos.
...
| Item | Função |
|---|---|
| Python 3 e JSON Lines | Simulação determinística da fila |
| Amazon SQS | Fila gerenciada numa evolução cloud |
| Visibility timeout | Ocultar temporariamente mensagem em processamento |
| Redrive policy | Definir a DLQ e `maxReceiveCount` |
| Dead-letter queue | Isolar mensagens que excedem tentativas |
| `unittest` e SVG | Teste e documentação visual |

## Conceitos aplicados
- retentativa, falha transitória e poison message;
- confirmação por exclusão após sucesso;
- contador de recebimentos e redrive;
- isolamento para diagnóstico e reprocessamento controlado.

## Pré-requisitos, custos e validação
Python 3.10+, custo local zero. Foram validados sucesso imediato, sucesso na segunda tentativa e DLQ após três tentativas. SQS, CloudWatch e Lambda podem gerar custos se implantados.

## Tecnologias relacionadas ainda não utilizadas
Não há filas AWS, IAM, SDK, visibility timeout real, backoff, Lambda, CloudWatch ou redrive de retorno.

## Referências oficiais
- [Dead-letter queues do Amazon SQS](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-dead-letter-queues.html)
- [Redrive de mensagens da DLQ](https://docs.aws.amazon.com/AWSSimpleQueueService/latest/SQSDeveloperGuide/sqs-configure-dead-letter-queue-redrive.html)

## Publicação
Rascunho local; nenhuma fila ou repositório remoto foi criado.
...
cd "004-sqs-retry-dlq"
```

Confirme que `pwd` termina em `004-sqs-retry-dlq`. Os caminhos relativos usados pelo código dependem disso.
