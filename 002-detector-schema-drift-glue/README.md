# Detector de schema drift antes do AWS Glue

> AWS 002 · iniciante/intermediário · contratos e Glue Data Catalog

Compara o cabeçalho de um CSV com o contrato esperado e classifica mudanças como compatíveis, aditivas, reordenação ou quebra. O exemplo detecta a nova coluna `coupon_code` antes de um crawler alterar o catálogo.

![Fluxo de schema](docs/arquitetura.svg)

## Executar e testar
```bash
python3 -m src.schema_guard
...
| Item | Função |
|---|---|
| Python 3, CSV e JSON | Inspeção local e contrato versionável |
| Schema contract | Definir colunas e tipos esperados |
| AWS Glue Crawler | Descoberta de schema numa evolução cloud |
| Glue Data Catalog | Catálogo que deve ser protegido de mudanças inesperadas |
| SchemaChangePolicy | Controlar atualização ou somente registrar mudanças |
| `unittest` e SVG | Testes e arquitetura visual |

## Conceitos aplicados
- data contract e schema drift;
- mudança aditiva versus mudança incompatível;
- validação antes da catalogação;
- política `LOG` como proteção do schema existente.

## Pré-requisitos, custos e validação
Python 3.10+, custo local zero. Testes confirmam coluna nova e coluna obrigatória ausente. Glue Crawler, Catalog e S3 podem gerar custos somente quando configurados manualmente.

## Tecnologias relacionadas ainda não utilizadas
Não usa AWS SDK, S3, crawler real, inferência de tipos por linhas, Schema Registry, alertas ou CI/CD.

## Referências oficiais
- [Impedir que um crawler altere um schema](https://docs.aws.amazon.com/glue/latest/dg/crawler-schema-changes-prevent.html)
- [Personalizar o comportamento do crawler](https://docs.aws.amazon.com/glue/latest/dg/crawler-configuration.html)

## Publicação
Rascunho local; nenhum catálogo, crawler ou repositório remoto foi alterado.
...
cd "002-detector-schema-drift-glue"
```

Confirme que `pwd` termina em `002-detector-schema-drift-glue`. Os caminhos relativos usados pelo código dependem disso.
