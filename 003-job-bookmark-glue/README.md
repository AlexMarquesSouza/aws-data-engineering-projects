# Job bookmark incremental no AWS Glue

> AWS 003 · intermediário · ETL incremental

Simula o estado que permite a um job processar somente linhas posteriores à última chave confirmada.

![Arquitetura](docs/arquitetura.svg)

## Executar e testar
```bash
python3 -m src.bookmark_job
...
| Item | Função |
|---|---|
| Python 3, CSV e JSON | Job e bookmark locais |
| AWS Glue ETL Job | Execução Spark serverless futura |
| Job bookmark | Registrar dados já processados |
| Bookmark key | Chave crescente usada na seleção |
| `job.init` e `job.commit` | Inicializar e confirmar estado no Glue real |
| `unittest` e SVG | Testes e arquitetura visual |

## Conceitos aplicados
- processamento incremental e estado persistente;
- limite inferior exclusivo;
- idempotência de reexecução;
- confirmação do estado após escrita da saída.

## Pré-requisitos, custos e validação
Python 3.10+, custo local zero. A primeira execução processa três clientes e a segunda, nenhum. AWS Glue e S3 podem gerar custos apenas numa implantação.

## Tecnologias relacionadas ainda não utilizadas
Não usa Spark, `transformation_ctx`, Glue real, JDBC, S3, concorrência, reset/rewind de bookmark ou deploy.

## Referências oficiais
- [Usar job bookmarks no AWS Glue](https://docs.aws.amazon.com/glue/latest/dg/programming-etl-connect-bookmarks.html)

## Publicação
Rascunho local; nenhum job, bucket ou repositório remoto foi criado.
...
cd "003-job-bookmark-glue"
```

Confirme que `pwd` termina em `003-job-bookmark-glue`. Os caminhos relativos usados pelo código dependem disso.
