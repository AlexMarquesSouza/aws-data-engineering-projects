# Advisor de workers para jobs AWS Glue

> AWS 013 · intermediário · capacidade e custo

Transforma volume de entrada e característica de memória em uma recomendação inicial de tipo, quantidade de workers e DPU total.

![Arquitetura](docs/arquitetura.svg)

```bash
python3 -m src.sizing
...
| Item | Função |
|---|---|
| Python / JSON | Estimador local reproduzível |
| AWS Glue ETL | Serviço serverless representado |
| G workers | Compute de propósito geral |
| R workers | Opção otimizada para memória |
| DPU | Unidade de capacidade usada no sizing |

## Conceitos de Engenharia de Dados aplicados

Capacity planning, workload profiling, memória versus CPU, paralelismo e right-sizing orientado a custo.

## Pré-requisitos e possíveis custos

Python 3.10+ e custo local zero. Jobs Glue são cobrados pelo tipo, quantidade e duração dos workers; nenhuma execução AWS ocorre.

## O que foi validado

Três testes cobrem job pequeno, memory-bound e backfill grande. A saída informa tipo, workers e DPU total.

## Tecnologias relacionadas ainda não utilizadas

Sem boto3, Auto Scaling, Flex execution, métricas CloudWatch, Spark UI, shuffle, bookmarks, cálculo monetário ou job real.

## Referências oficiais

- [Tipos de worker no objeto Job](https://docs.aws.amazon.com/glue/latest/webapi/API_Job.html)
- [Configurar IAM para Glue](https://docs.aws.amazon.com/glue/latest/dg/configure-iam-for-glue.html)
...
cd "013-glue-worker-sizing-advisor"
```

Confirme que `pwd` termina em `013-glue-worker-sizing-advisor`. Os caminhos relativos usados pelo código dependem disso.
