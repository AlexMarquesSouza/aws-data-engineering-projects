# Workflow ETL com Retry e Catch

> AWS 005 · intermediário · orquestração resiliente

Simula Extract, Transform e Load. Uma falha transitória é recuperada na segunda tentativa; uma falha permanente esgota três tentativas e segue para notificação via `Catch`.
...
python3 -m src.workflow --mode permanent
...
| Item | Função |
|---|---|
| Python 3 | Simulador local da máquina de estados |
| AWS Step Functions | Orquestração gerenciada futura |
| Amazon States Language | Definição declarativa incluída no projeto |
| Retry e BackoffRate | Recuperar falhas transitórias |
| Catch | Direcionar falhas permanentes |
| Lambda | Recurso ilustrativo do estado Task |
| `unittest` e SVG | Testes e arquitetura visual |

## Conceitos aplicados
- máquina de estados e histórico de execução;
- retry limitado e backoff exponencial;
- distinção entre falha transitória e permanente;
- fallback explícito após esgotar tentativas.

## Pré-requisitos, custos e validação
Python 3.10+, custo local zero. Foram validados sucesso na segunda tentativa e fallback após três falhas. Step Functions e Lambda podem gerar custos se implantados.

## Tecnologias relacionadas ainda não utilizadas
Não há conta AWS, Lambda real, IAM, deploy, espera temporal, Map, Parallel, métricas ou notificações externas.
...
- [O que é AWS Step Functions](https://docs.aws.amazon.com/step-functions/latest/dg/welcome.html)

## Publicação
Rascunho local; a máquina de estados não foi implantada.
...
cd "005-step-functions-retry-catch"
```

Confirme que `pwd` termina em `005-step-functions-retry-catch`. Os caminhos relativos usados pelo código dependem disso.
